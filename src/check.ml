  ...
  struct
    ...
    let check_redefinition decls =
        let f (nonterms, imports as r) (decl, loc) =
            match decl with
                A.Bind (_, sym, _) ->
                    if exists ((=) sym) nonterms then error loc (sprintf "redefinition of non-terminal symbol '%s'" sym)
                    else sym :: nonterms, imports

              | A.Import (Some sym, _) ->
                    if exists ((=) sym) imports then error loc (sprintf "redefinition of import path symbol '%s'" sym)
    ...
    let check_basic =
        ...
        let rec check_decls env prefixes decls =
            let f (decl, _) =
                match decl with
                    A.Bind (_, _, p)     -> check_prod env prefixes p
                  | A.Import (_, decls') -> check_decls env prefixes decls'
            in
                iter f decls

        and check_prod env prefixes (A.Prod seqs, _) = iter (check_seq env prefixes) seqs

        and check_seq env prefixes (A.Seq (_, atoms), _) =
            check_useless_permutations atoms;
            iter (check_atom env prefixes) atoms
